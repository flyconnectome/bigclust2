...
class NglViewer:
    ...
    max_threads : int
        Maximum number of threads to use for loading neurons.
    viewer_kwargs : dict
        Keyword arguments to pass to the octarine Viewer.
    ...
    def set_neuropil_mesh(self, neuropil_mesh, neuropil_source=None):
        ...
        if isinstance(neuropil_mesh, str):
            # Re-use as source if not explicitly provided
            if not self.neuropil_source and "@" in neuropil_mesh:
                self.neuropil_source = neuropil_mesh
        ...
        if "neuropil" in self.viewer.objects:
            self.viewer.remove_objects("neuropil")
    ...
    def set_data(self, data):
        ...
        data : pandas.DataFrame
            ...
            `source` column that contains a URL for CloudVolume. Optional:
              ...
              - 'dataset' column to distinguish neurons with same ID in different datasets
    ...
    def set_default_colors(self):
        ...
        if "_color" in self.data.columns:
            self._colors = self.data._color.to_dict()
    ...
    def force_single_render(self):
        ...
        self.viewer.renderer.render(self.viewer.scene, self.viewer.camera, flush=True)
        self.viewer.renderer.render(
            self.viewer.overlay_scene, self.viewer.overlay_camera
        )
    ...
    def check_futures(self):
        ...
        if has_futures and len(self.futures) == 0:
            if not self.n_failed:
                self.viewer.show_message(
                    "All neurons loaded", duration=2, position="bottom-right", color="w"
                )
            else:
                self.viewer.show_message(
...
def load_neuropil_mesh(x):
    ...
    x : str
        This can be either:
         - a filepath to a local file (e.g. "/path/to/mesh.ply")
         - a URL to a mesh file (e.g. "https://example.com/mesh.ply")
         - a neuroglancer source string with the format "id@source",
           where `id` is the segment ID and `source` is a CloudVolume URL
           (e.g. "12345@precomputed://...").
    ...
    if isinstance(mesh, tm.Scene):
        ...
        if len(mesh) != 1:
            raise ValueError(
                f"Got a scene with {len(mesh)} geometries, "
