                ...
                if k < n and s[k] in "([":
                    ...
                    while k < n:
                        ...
                        while k < n and s[k].isspace():
                            k += 1
                        if k >= n:
                            raise ValueError(f"Unclosed list in 'in' operator: expected '{closing}' before end of string")
                        if s[k] == closing:
                            k += 1
                            break
                        ...
                        if s[k] in "'\"":
                            ...
                            if not found_closing_quote:
                                raise ValueError(f"Unclosed string in list at pos {k}: expected '{quote}'")
                            list_items.append("".join(buf))
                        elif s[k].isdigit() or (s[k] == "." and k + 1 < n and s[k + 1].isdigit()):
                            start = k
                            has_dot = False
                            ...
                            raise ValueError(f"Unexpected token in list at pos {k}: {s[k]}")
                    ...
                    raise ValueError(f"'in' operator must be followed by a list in parentheses or brackets at pos {j}")
            else:
